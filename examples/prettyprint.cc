...
// loosely based on a greatly simplified version of BeautifulSoup4 decode()
// routine
//
...
static std::string nonbreaking_inline =
    "|a|abbr|acronym|b|bdo|big|cite|code|dfn|em|font|i|img|kbd|nobr|s|small|"
    "span|strike|strong|sub|sup|tt|";
static std::string empty_tags =
    "|area|base|basefont|bgsound|br|command|col|embed|event-source|frame|hr|"
    "image|img|input|keygen|link|menuitem|meta|param|source|spacer|track|wbr|";
...
static std::string build_attributes(GumboAttribute* at, bool no_entities) {
  ...
  if ((!std::string(at->value).empty()) ||
    ...
    if (no_entities) {
      ...
      atts.append(substitute_xml_entities_into_attributes(
          quote, std::string(at->value)));
    }
...
static std::string prettyprint(
    GumboNode*, int lvl, const std::string indent_chars);

// prettyprint children of a node
// may be invoked recursively

static std::string prettyprint_contents(
    GumboNode* node, int lvl, const std::string indent_chars) {
  std::string contents = "";
  ...
  for (unsigned int i = 0; i < children->length; ++i) {
    ...
    if (child->type == GUMBO_NODE_TEXT) {
      ...
      if (no_entity_substitution) {
        ...
        val =
            substitute_xml_entities_into_text(std::string(child->v.text.text));
      }
      ...
      if ((nonbreaking_inline.find(childkey) != std::string::npos) &&
          (contents.length() > 0)) {
        ltrim(val);
      }
...
int main(int argc, const char** argv) {
  ...
  GumboOutput* output =
      gumbo_parse_with_options(&options, contents.data(), contents.length());
